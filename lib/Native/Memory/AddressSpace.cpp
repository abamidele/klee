...
AddressSpace::AddressSpace(void) :
		page_to_map(256), wnx_page_to_map(256), min_addr(
				std::numeric_limits<uint64_t>::max()), addr_mask(
				GetAddressMask()), invalid(
...
AddressSpace::AddressSpace(const AddressSpace &parent) :
		maps(parent.maps.size()), page_to_map(parent.page_to_map.size()), wnx_page_to_map(
				...
				parent.page_is_readable), page_is_writable(
				parent.page_is_writable), page_is_executable(
...
bool AddressSpace::TryRead(uint64_t addr_, void *val_out, size_t size) {
	...
	if (address.must_be_fe == klee::native::special_malloc_byte) {
		...
		if (alloc_list_pair == alloc_lists.end()) {
			LOG(ERROR)
					<< "Invalid Memory Access In Malloced Memory Region On Read";
			return false;
		}
...
bool AddressSpace::TryWrite(uint64_t addr_, const void *val, size_t size) {
	...
	if (address.must_be_fe == klee::native::special_malloc_byte) {
		...
		if (alloc_list_pair == alloc_lists.end()) {
			LOG(ERROR)
					<< "Invalid Memory Access In Malloced Memory Region On Write";
			return false;
		}
		for (size_t offset = 0; offset < size; ++offset) {
...
static std::vector<MemoryMapPtr> RemoveRange(
		const std::vector<MemoryMapPtr> &ranges, uint64_t base,
		uint64_t limit) {
...
void AddressSpace::AddMap(uint64_t base_, size_t size, const char *name,
	...
	if (unlikely(is_dead)) {
		LOG(ERROR) << "Trying to map range [" << std::hex << base << ", "
				<< limit << ") in destroyed address space." << std::dec;
		return;
	}
	...
	if (old_ranges.size() < maps.size()) {
		LOG(INFO) << "New map [" << std::hex << base << ", " << limit << ")"
				<< " overlapped with " << std::dec
				<< (maps.size() - old_ranges.size()) << " existing maps";
	}
...
void AddressSpace::RemoveMap(uint64_t base_, size_t size) {
	...
	auto old_ranges = RemoveRange(maps, base, limit);
	if (old_ranges.size() < maps.size()) {
		LOG(INFO) << "New invalid map [" << std::hex << base << ", "
				<< limit << ")" << " overlapped with " << std::dec
				<< (maps.size() - old_ranges.size()) << " existing maps";
	}
	maps.swap(old_ranges);
	maps.push_back(new_map);
...
void AddressSpace::CreatePageToRangeMap(void) {
	...
	for (const auto &map : maps) {
		...
		for (auto addr = base_address; addr < limit_address; addr +=
				kPageSize) {
...
void AddressSpace::LogMaps(std::ostream &os) const {
	...
	for (const auto &range : maps) {
		...
		ss << "  [" << std::hex << std::setw((int32_t) (arch->address_size / 4))
				...
				<< std::setw((int32_t) (arch->address_size) / 4)
