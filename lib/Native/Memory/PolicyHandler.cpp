...
bool ReportErrorPolicyHandler::HandleHeapReadOverflow(AddressSpace *mem,
    const Address& address, uint8_t *byte_out) {
...
bool ReportErrorPolicyHandler::HandleInvalidOutOfBoundsHeapRead(
    AddressSpace *mem, const Address& address) {
...
uint64_t ProxyPolicyHandler::HandleBadRealloc(AddressSpace *mem,
    const Address& address, size_t alloc_size, uint64_t err_type) {
...
bool SymbolicBufferPolicy::HandleHeapWriteUnderflow(AddressSpace *mem, const Address& address) {
  return false;
}
...
bool SymbolicBufferPolicy::HandleHeapReadUnderflow(AddressSpace *mem, const Address& address) {
  return false;
}
