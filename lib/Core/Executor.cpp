...
cl::OptionCategory TerminationCat(
...
cl::opt<unsigned> MaxForks(
    "max-forks", cl::desc("Only fork this many times (default=-1 (off))"),
...
void Executor::AddInitialTask(const std::string &state, const uint64_t pc,
  ...
  LOG(INFO)
      << "Initialized snapshotted task state";
}
...
Executor::StatePair Executor::fork(ExecutionState &current, ref<Expr> condition,
  ...
  if (res == Solver::True) {
    ...
    if (it != seedMap.end()) {
      ...
      for (std::vector<SeedInfo>::iterator siit = seeds.begin(), siie = seeds
          .end(); siit != siie; ++siit) {
        ref<ConstantExpr> res;
        bool success = solver->getValue(current,
                                        siit->assignment.evaluate(condition),
  ...
  if (f && f->isDeclaration()) {
    ...
    if (!f->isVarArg()) {
      ...
      MemoryObject *mo = sf.varargs = memory->allocate(
          size, true, false, state.prevPC->inst,
  ...
  switch (i->getOpcode()) {
    ...
    case Instruction::Switch: {
      ...
      if (ConstantExpr *CE = dyn_cast<ConstantExpr>(cond)) {
        ...
        for (std::map<ref<Expr>, BasicBlock *>::iterator it = expressionOrder
            .begin(), itE = expressionOrder.end(); it != itE; ++it) {
    ...
    case Instruction::GetElementPtr: {
      ...
      ref<Expr> base = eval(ki, 0, state).value;
      for (std::vector<std::pair<unsigned, uint64_t> >::iterator it = kgepi
          ->indices.begin(), ie = kgepi->indices.end(); it != ie; ++it) {
    ...
    case Instruction::Trunc: {
      ...
      ref<Expr> result = ExtractExpr::create(
          eval(ki, 0, state).value, 0, getWidthForLLVMType(ci->getType()));
      bindLocal(ki, state, result);
      break;
    }
    ...
    case Instruction::AtomicCmpXchg:
      terminateStateOnExecError(
          state, "Unexpected AtomicCmpXchg instruction, should be "
          "lowered by LowerAtomicInstructionPass");
      break;
      // Other instructions...
      // Unhandled
...
void Executor::computeOffsets(KGEPInstruction *kgepi, TypeIt ib, TypeIt ie) {
  ref<ConstantExpr> constantOffset = ConstantExpr::alloc(
...
void Executor::callExternalFunction(ExecutionState &state, KInstruction *target,
  ...
  for (std::vector<ref<Expr> >::iterator ai = arguments.begin(), ae = arguments
      .end(); ai != ae; ++ai) {
